...
//             if (!agent.pathPending && agent.remainingDistance <= agent.stoppingDistance + 0.5f)
//             {
//                 if (pointWaitTimer > 0) { pointWaitTimer -= Time.deltaTime; LookAround(); }
//                 else
//                 {
...
//     void LookAround() { anim.SetInteger(animStateHash, 4); transform.Rotate(0, 40f * Time.deltaTime, 0); }
...
//         if (clipsToPlay != null && clipsToPlay.Length > 0) { audioSource.PlayOneShot(clipsToPlay[Random.Range(0, clipsToPlay.Length)]); }
//     }

//     void StartJumpscare()
//     {
//         if (isSequenceRunning || isGameOver) return;
//         isSequenceRunning = true; isJumpscaring = true; currentState = EnemyState.Jumpscare;
//         if (audioSource.isPlaying) audioSource.Stop();
//         agent.isStopped = true;
//         anim.SetInteger(animStateHash, 3); anim.SetTrigger("Jumpscare");
//         jumpscareTimer = jumpscareDuration;
//         if (enemyLight != null) { enemyLight.enabled = true; enemyLight.transform.localPosition = originalLightPosition + jumpscareLightOffset; }
...
//         playerCamera.gameObject.SetActive(false); jumpscareCamera.gameObject.SetActive(true);
//     }
...
//         if (blackoutPanel != null) { blackoutPanel.SetActive(true); Image img = blackoutPanel.GetComponent<Image>(); if (img != null) img.color = Color.black; }
...
//         if (playerPrisonSpawn != null) { player.position = playerPrisonSpawn.position; player.rotation = playerPrisonSpawn.rotation; }
//         yield return new WaitForFixedUpdate(); yield return new WaitForEndOfFrame();
//         if (playerController != null) playerController.enabled = true;
//         jumpscareCamera.gameObject.SetActive(false); playerCamera.gameObject.SetActive(true);
//         isJumpscaring = false; if (playerCamScript != null) playerCamScript.enabled = true;
//         if (enemyLight != null) { enemyLight.transform.localPosition = originalLightPosition; enemyLight.intensity = originalLightIntensity; enemyLight.color = searchColor; }
//         if (blackoutPanel != null) { Image img = blackoutPanel.GetComponent<Image>(); if (img != null) { float a = 1f; while (a > 0f) { a -= Time.deltaTime * fadeInSpeed; img.color = new Color(0, 0, 0, a); yield return null; } } blackoutPanel.SetActive(false); }
//         if (staringVoiceline != null) { audioSource.clip = staringVoiceline; audioSource.Play(); }
...
//     void EndJumpscare() { isGameOver = true; Time.timeScale = 0f; Cursor.lockState = CursorLockMode.None; Cursor.visible = true; gameOverScreen.SetActive(true); }
